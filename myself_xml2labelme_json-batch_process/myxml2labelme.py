...
## 批量获得xml地址
def batch_procss(input_dir):
    file_dir = os.path.split(input_dir)[0]
    data=[d for d in os.listdir(file_dir)] # os.listdir可以列出文件和目录
    for i in range(len(data)):
        print(file_dir+'/'+data[i])
        convert(file_dir+'/'+data[i])
