...
for i in range(len(jsonINDEX)):#遍历所有的labelme 标准的json，载入并转换
    #ind='./test_img/val_img/s{index}.json'.format(index=i)
    #ind='E:/Data_Set/mask-cai/four_cls/s{index}.json'.format(index=i)
    ind='./data/{}'.format((jsonINDEX[i]))
    labelme_json.append(ind)


#labelme_json=glob.glob('/content/drive/My Drive/VOCMMdete/mmdetection/cc/*.json')# 这样子无法指定categorie的ID
lab = labelme2coco(labelme_json,'./sileiv2_with_area1722.json')#调用labelme2coco类
